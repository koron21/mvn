...
Shader "Langvv/Rim_Scroll_U_Additive_Diffuse_Normal" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_108.rg, _Normal))).rgb;
...
////// Emissive:
                ...
                float3 finalColor = diffuse * (tex2D(_TextureDiffuse,TRANSFORM_TEX(node_108.rg, _TextureDiffuse)).rgb*_MainColor.rgb) + emissive;
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_109.rg, _Normal))).rgb;
                float3 normalDirection = normalize( mul( normalLocal, tangentTransform ) );
...
/////// Diffuse:
                ...
                float3 finalColor = diffuse * (tex2D(_TextureDiffuse,TRANSFORM_TEX(node_109.rg, _TextureDiffuse)).rgb*_MainColor.rgb);
/// Final Color:
