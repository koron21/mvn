...
////// Emissive:
                ...
                float3 emissive = ((tex2D(_TextureDiffuse,TRANSFORM_TEX(i.uv0.rg, _TextureDiffuse)).rgb*_MainColor.rgb)+(pow((1.0-max(0,dot(i.normalDir, viewDirection))),_RimPower)*((tex2D(_RimTexture,TRANSFORM_TEX((i.uv0.rg+(node_95.g*_RimSpeed)*float2(1,0)), _RimTexture)).rgb*_RimColor.rgb)*_RimIntensity)));
